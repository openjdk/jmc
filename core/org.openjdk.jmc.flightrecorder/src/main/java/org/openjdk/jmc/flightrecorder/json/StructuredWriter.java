...
/**
 * This class is a hacked-up combination of the XML serializer in the JMC RecordingPrinter
 * and the `jfr` command's StructuredWriter.
 * <p/>
 * It provides a set of utility methods for serialising arbitrary values to JSON, with special logic for JMC-specific
 * types and Java primitives.
 * <p/>
 * @see org.openjdk.jmc.flightrecorder.RecordingPrinter
 * @see <a href="https://github.com/openjdk/jdk11/blob/master/src/jdk.jfr/share/classes/jdk/jfr/internal/cmd/StructuredWriter.java">jdk.jfr.internal.cmd.StructuredWriter</a>
 */
abstract class StructuredWriter {
    ...
    private String stringifyType(IMCType type) {
        return formatPackage(type.getPackage()) +
                "." + //$NON-NLS-1$
                type.getTypeName();
    }

    protected String stringifyMethod(IMCMethod method) {
        return formatPackage(method.getType().getPackage()) +
                "." + //$NON-NLS-1$
                method.getType().getTypeName() +
                "#" + //$NON-NLS-1$
                method.getMethodName() +
                method.getFormalDescriptor();
    }
